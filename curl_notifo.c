 ...
 */
int
__notifo_response (void *ptr, size_t size, size_t nmemb, void *userdata)
{
  return (size * nmemb);
}
 ...
 */
int
curl_notifo (const char *username, const char *apiKey, char *to, char *label,
	     char *title, char *message, char *url)
{
  ...
  if (curl)
    {
      ...
      if (to)
	{
	  curl_formadd (&formpost, &lastptr, CURLFORM_COPYNAME, NOTIFO_TO_KEY,
			CURLFORM_COPYCONTENTS, to, CURLFORM_END);
	}
      if (label)
	{
	  curl_formadd (&formpost, &lastptr, CURLFORM_COPYNAME,
			NOTIFO_LABEL_KEY, CURLFORM_COPYCONTENTS, label,
			CURLFORM_END);
	}
      if (title)
	{
	  curl_formadd (&formpost, &lastptr, CURLFORM_COPYNAME,
			NOTIFO_TITLE_KEY, CURLFORM_COPYCONTENTS, title,
			CURLFORM_END);
	}
      if (message)
	{
	  curl_formadd (&formpost, &lastptr, CURLFORM_COPYNAME,
			NOTIFO_MESSAGE_KEY, CURLFORM_COPYCONTENTS, message,
			CURLFORM_END);
	}
      if (url)
	{
	  curl_formadd (&formpost, &lastptr, CURLFORM_COPYNAME,
			NOTIFO_URL_KEY, CURLFORM_COPYCONTENTS, url,
			CURLFORM_END);
	}
